---
name: vault-audit
description: "Comprehensive multi-pass audit on a second brain vault. 22 passes across 4 groups: size & tokens, wiki linting (dead links, orphans, duplicates, routing, stubs), skills (vault duplication, frontmatter, references depth), and vault hygiene (frontmatter compliance, filename quality, em dashes, code-style rules in CLAUDE.md, README hygiene). Presents a clean markdown summary, lets the user approve fixes per category via AskUserQuestion, applies only what's approved, saves a dated report to Intelligence/decisions/. Encodes 5 frameworks (Anthropic CLAUDE.md, Karpathy LLM Wiki lint, Caveman compression, Chroma context rot, Anthropic Memory) plus practitioner field notes. TRIGGERS: vault audit, lint vault, vault health, vault health check, audit my brain, brain audit, second brain audit, clean up vault, is my vault bloated, vault dead links, vault token cost, find duplicate context, is my vault healthy, find orphan notes, frontmatter compliance, em dash audit. REQUIREMENT: run from the user's vault root (folder containing the root CLAUDE.md and Context/)."
---
